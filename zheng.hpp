...
#include "cpp-btree/btree/map.h"
#pragma GCC diagnostic pop

#include "combinatorics.hpp"

constexpr int THREADS = 16;
constexpr long long RAM = 24 * (1LL << 30);

std::ofstream debug;


namespace timer {
    btree::map<std::string, decltype(std::chrono::steady_clock::now())> begin_time;
...
auto compute_r_and_s(T d) {
    ...
    parallel_iterate_on_secondary_partitions(THREADS, p_table, d, [&](int t, auto omega) {
        ...
        I r = (k % 2) ? (1) : (-1);
        r *= factorial[k - 1];
        btree::map<std::tuple<T, uint32_t>, uint32_t> multiplicities;
